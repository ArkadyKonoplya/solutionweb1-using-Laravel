...
class LogController extends Controller {
	...
	function getUpdate(Request $request, $id = null)
	{
		...
		$this->data['id'] = $id;
		return view('log.form',$this->data);
	}
	...
	public function getShow( Request $request, $id = null)
	{
		...
		if($row)
		{
			...
			return view('log.view',$this->data);
		} else {
			return Redirect::to('log')->with('messagetext',\Lang::get('core.norecord'))->with('msgstatus','error');
		}
	}

	function postCopy( Request $request)
	{
	    foreach(\DB::select("SHOW COLUMNS FROM tb_logs ") as $column)
        {
		...
		if(count($request->input('ids')) >=1)
		{
			...
			$sql = "INSERT INTO tb_logs (".implode(",", $columns).") ";
			$sql .= " SELECT ".implode(",", $columns)." FROM tb_logs WHERE auditID IN (".$toCopy.")";
			\DB::insert($sql);
			return Redirect::to('log')->with('messagetext',\Lang::get('core.note_success'))->with('msgstatus','success');
		} else {

			return Redirect::to('log')->with('messagetext',\Lang::get('core.note_selectrow'))->with('msgstatus','error');
		}
	...
	function postSave( Request $request)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('tb_log');

			$id = $this->model->insertRow($data , $request->input('auditID'));

			if(!is_null($request->input('apply')))
			{
				$return = 'log/update/'.$id.'?return='.self::returnUrl();
			} else {
				$return = 'log?return='.self::returnUrl();
			}

			// Insert logs into database
			if($request->input('auditID') =='')
			{
			...
			return Redirect::to('log/update/'. $request->input('auditID'))->with('messagetext',\Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public static function display( )
	{
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('log.public.view',$data);
			}
			...
			return view('log.public.index',$data);
		}
	...
	function postSavepublic( Request $request)
	{
		...
		$validator = Validator::make($request->all(), $rules);
		if ($validator->passes()) {
			$data = $this->validatePost('tb_logs');
