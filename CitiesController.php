...
class CitiesController extends Controller {
	...
	public function postData( Request $request)
	{
		...
		$pagination->setPath('cities/data');
		...
		return view('cities.table',$this->data);
	...
	function getUpdate(Request $request, $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] 		= $this->model->getColumnTable('def_city');
		}
		$this->data['setting'] 		= $this->info['setting'];
		...
		return view('cities.form',$this->data);
	}
	...
	public function getShow( Request $request , $id = null)
	{
		...
		if($row)
		{
			...
			if($print =='true')
			{
				$data['html'] = view('cities.view', $this->data)->render();
				return view('layouts.blank',$data);
			} else {
				return view('cities.view',$this->data);
			}
	...
	function postCopy( Request $request)
	{
	    ...
	    foreach(\DB::select("SHOW COLUMNS FROM def_city ") as $column)
        {
		...
		if(count($request->input('ids')) >=1)
		{
			...
			$sql = "INSERT INTO def_city (".implode(",", $columns).") ";
			$sql .= " SELECT ".implode(",", $columns)." FROM def_city WHERE cityID IN (".$toCopy.")";
			\DB::insert($sql);
			return response()->json(array(
				'status'=>'success',
	...
	function postSave( Request $request, $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('def_city');

			$id = $this->model->insertRow($data , $request->input('cityID'));
	...
	function postSavepublic( Request $request)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('def_city');
			 $this->model->insertRow($data , $request->input('cityID'));
