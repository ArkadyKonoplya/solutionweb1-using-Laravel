  ...
  static public function travelersDetail( $traveler = '', $packageID)
	{
		...
		if($traveler !='')
		{
			...
			$booking_IDs = \DB::table('invoice_products')
					...
					// var_dump($travellerID_bytra);exit;
...
// var_dump($InvTotal);exit;
			...
			$travelersDetail .= "<div class='col-md-3'><a href='".url('travellers/show')."/".$traveler."'>".$nameandsurname."</a></div><div class='col-md-3'>".number_format((float)$payment, 2)." ".$currency->symbol."</div><div class='col-md-3'>".number_format((float)$unpaid_sum, 2)." ".$currency->symbol."</div><div class='col-md-3'>".InvoiceStatus::package_payments($payment, $InvTotal, $traveller_bytra)."</div>";
		}
