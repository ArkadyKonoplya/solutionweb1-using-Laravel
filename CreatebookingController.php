...
class CreatebookingController extends Controller {
	...
	public function getShow( Request $request, $id = null)
	{
      ...
      if(!is_null($request->input('pdf')))
			{
					if($row->policyandterms != 0)
					$this->data['termsconditions'] = Tandc::find($row->policyandterms);
	...
	public function postDelete( Request $request)
	{
		...
		if(count($request->input('ids')) >=1)
		{
			...
			foreach($ids as $id){
				\DB::table('book_tour')->where('bookingID', '=', $id)->delete();
				\DB::table('book_car')->where('bookingID', '=', $id)->delete();
				\DB::table('book_extra')->where('bookingID', '=', $id)->delete();
				\DB::table('book_flight')->where('bookingID', '=', $id)->delete();
				\DB::table('book_hotel')->where('bookingID', '=', $id)->delete();
				\DB::table('book_room')->where('bookingID', '=', $id)->delete();
			}
			// var_dump($ids);exit;
